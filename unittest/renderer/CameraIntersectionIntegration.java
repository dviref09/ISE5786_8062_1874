...
class CameraIntersectionIntegration {
     ...
     * the given geometry is as expected.
     * @param camera The camera constructing the rays.
     * @param geometry The geometric body getting intersected by the rays.
    ...
    void testCameraRayPlaneIntegration() {
        ...
        assertIntersectionsCount(testCamera, testPlane, 9, "Plane slightly titled towards the camera (9 " +
                "intersections)");
        
        // EP03: Plane tilted, misses some pixels (6 intersections)
        testPlane = new Plane(new Point(-5, 0, 0), new Vector(1, 0, 1));
        assertIntersectionsCount(testCamera, testPlane, 6, "Plane titled away from the camera, missing some pixels (9" +
                " intersections)");
