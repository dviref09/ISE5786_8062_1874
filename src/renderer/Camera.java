...
public final class Camera implements Cloneable {
	...
	/**
	 * The amount of threads to be used in the rendering process: 0 - no multithreading -1 - parallel stream 1+ - raw
	 * threads
	 */
	...
	/**
	 * Creates a ray through the center of a pixel in the view plane For the indexes the center is considered (0,0)
	...
	/**
	 * Private helper method for creating a ray through a pixel, calculating its color and then paints the corresponding
	 * pixel in imageWriter
	...
	public static class Builder {
		 ...
		 * @param target A point in the direction of the camera, by this point the direction vector of the camera will be
		 *               calculated
		 * @param up     The direction of the top of the camera (affects rotation)
		 * @return The same builder for chaining methods
		 */
		 ...
		 * @param threads The amount of threads to be used: 0 - no multithreading -1 - parallel stream -2 - auto raw threads 1+
		 *                - raw threads
		 * @return The same builder for chaining methods
		 */
		...
		/**
		 * Checks whether the resolution values are valid or not Adds the imageWriter with the resolution data
		...
		/**
		 * Checks if there is enough data about the camera position Enough data is: Mandatory: camera's position and at least
		 * one of below: 1) Target point 2) vTo After that calculates the remaining data
		...
		/**
		 * Checks if the view plane data is valid And then calculates the center point of the view plane and the pixel
		 * dimensions
		 * @throws IllegalArgumentException If the width or the height or the distance of the view plane are not positive
		 */
		...
		/**
		 * Checks if there is a ray tracer and if there isn't then sets a default ray tracer: simple and scene called "test"
		 */
		...
		/**
		 * Calculates the vUp, vRight vectors. All the vectors will be normalized
		...
		/**
		 * Calculates the missing data of the view plane: The center point and the pixel dimensions
		 */
