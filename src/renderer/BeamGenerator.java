...
final class BeamGenerator {
     ...
     */
    static List<Ray> generateBeam(Point origin, List<Point> target, boolean switchDirection) {
    ...
    /**
     * Static method for generating beam of rays with shifting in the axis of the normal
     * @param origin The usual origin point of the rays
     * @param target The usual target points of the rays
     * @param switchDirection the direction of the ray, false for from origin to target, true for from target to origin
     * @return A list of the rays
     */
    static List<Ray> generateBeam(Point origin, List<Point> target, Vector normal, boolean switchDirection) {
        List<Ray> beam = new LinkedList<>();
        for (Point targetPoint : target) {
            Vector direction = (switchDirection ? origin.subtract(targetPoint) : targetPoint.subtract(origin));
            beam.add(new Ray(origin, direction, normal));
        }
     ...
     */
    static Color averageColor(List<Ray> beam, RayTracerBase tracer) {
